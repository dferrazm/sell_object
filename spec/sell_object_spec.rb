...
describe SellObject do
	...
	describe 'ClassMethods' do
		describe '#selling_through' do		
			context 'passing invalid arguments' do
				it 'raises ArgumentError with an invalid engine' do
					expect { TargetClass.selling_through :foo }.to raise_error ArgumentError, 'invalid shopping engine foo'
				end

				it 'raises ArgumentError with a blank arguments' do
					expect { TargetClass.selling_through }.to raise_error ArgumentError, 'must pass at least one shopping engine'
				end
			end

			context 'shopping_uol' do			
				it 'includes the module SellObject::ShoppingUol in the target class' do
					TargetClass.selling_through :shopping_uol
